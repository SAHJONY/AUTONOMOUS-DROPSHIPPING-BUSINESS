...
export async function GET(req: Request, { params }: { params: Promise<{ orgId: string }> }) {
  ...
  const productionGate = productionCatalogReadiness(products);
  return json({
    checks: [
      ...
      { id:"catalog", ready:productionGate.productionReady, label:`Production-complete products (${productionGate.ready}/25)` },
