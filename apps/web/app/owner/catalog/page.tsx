...
type Product = { id:string; title:string; description:string; category?:string; cost:number; price:number; status:string; source?:string; supplier?:string; supplier_url?:string; supplier_sku?:string; supplier_contact?:string; country_of_origin?:string; provenance_notes?:string; reorder_url?:string; reorder_moq?:number; reorder_case_pack?:number; reorder_lead_time_days?:number; provenance_verified_at?:string; shipping_weight_oz?:number; package_length_in?:number; package_width_in?:number; package_height_in?:number; fragile?:boolean; liquid?:boolean; shopify_id?:number; storefront_url?:string; sku?:string; inventory_quantity?:number; inventory_policy?:"deny"|"continue"; image_url?:string; images?:string[]; video_url?:string; audio_url?:string };
  ...
  async function addProduct(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/orgs/${oid}/owner-catalog`, { method:"POST", headers, body:JSON.stringify({
        ...
        images, inventory_quantity:Number(form.get("inventory_quantity") ?? 0), inventory_policy:form.get("inventory_policy"), shipping_weight_oz:Number(form.get("shipping_weight_oz") ?? 0), package_length_in:Number(form.get("package_length_in") ?? 0), package_width_in:Number(form.get("package_width_in") ?? 0), package_height_in:Number(form.get("package_height_in") ?? 0), fragile:form.get("fragile") === "on", liquid:form.get("liquid") === "on", sync_shopify:form.get("sync_shopify") === "on", publish:form.get("publish") === "on",
      }) });
      const body = await res.json().catch(() => ({})); if (!res.ok) throw new Error(body.detail ?? `Request failed (${res.status})`);
  ...
  async function saveProductEdits(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/orgs/${oid}/owner-catalog`, { method:"PATCH", headers, body:JSON.stringify({
        ...
        inventory_quantity:Number(form.get("inventory_quantity") ?? 0), inventory_policy:form.get("inventory_policy"), shipping_weight_oz:Number(form.get("shipping_weight_oz") ?? 0), package_length_in:Number(form.get("package_length_in") ?? 0), package_width_in:Number(form.get("package_width_in") ?? 0), package_height_in:Number(form.get("package_height_in") ?? 0), fragile:form.get("fragile") === "on", liquid:form.get("liquid") === "on", video_url:form.get("video_url"), audio_url:form.get("audio_url"),
    ...
    <div className={styles.content}>
      ...
      <section className={styles.section} id="add-product">
        ...
        <form onSubmit={addProduct} onPaste={pasteProductMedia} className={`${styles.card} ${styles.form}`}>
          ...
          <div className={`${styles.result} ${styles.full}`}><strong>Datos de envío</strong><div className={styles.productMeta}>Registra el paquete ya preparado para calcular costos y elegir embalaje correctamente.</div></div><label>Peso empacado (oz)<input className={styles.input} name="shipping_weight_oz" type="number" min="0" step="0.1"/></label><label>Largo (in)<input className={styles.input} name="package_length_in" type="number" min="0" step="0.1"/></label><label>Ancho (in)<input className={styles.input} name="package_width_in" type="number" min="0" step="0.1"/></label><label>Alto (in)<input className={styles.input} name="package_height_in" type="number" min="0" step="0.1"/></label><label><input type="checkbox" name="fragile"/> Frágil</label><label><input type="checkbox" name="liquid"/> Contiene líquido</label>
      ...
      {editingProduct && <section className={styles.section} id="edit-product">
        ...
        <form onSubmit={saveProductEdits} className={`${styles.card} ${styles.form}`}>
          ...
          <div className={`${styles.result} ${styles.full}`}><strong>Datos de envío</strong></div><label>Peso empacado (oz)<input className={styles.input} name="shipping_weight_oz" type="number" min="0" step="0.1" defaultValue={editingProduct.shipping_weight_oz}/></label><label>Largo (in)<input className={styles.input} name="package_length_in" type="number" min="0" step="0.1" defaultValue={editingProduct.package_length_in}/></label><label>Ancho (in)<input className={styles.input} name="package_width_in" type="number" min="0" step="0.1" defaultValue={editingProduct.package_width_in}/></label><label>Alto (in)<input className={styles.input} name="package_height_in" type="number" min="0" step="0.1" defaultValue={editingProduct.package_height_in}/></label><label><input type="checkbox" name="fragile" defaultChecked={editingProduct.fragile}/> Frágil</label><label><input type="checkbox" name="liquid" defaultChecked={editingProduct.liquid}/> Contiene líquido</label>
