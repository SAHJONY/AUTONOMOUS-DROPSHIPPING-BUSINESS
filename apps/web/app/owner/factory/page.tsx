...
export default function FactoryPage() {
  ...
  return <main className={styles.shell}>
    ...
    <div className={styles.content}>
      ...
      <section className={styles.section}><div className={styles.sectionHead}><div><span>PASO 2 · BOM Y COSTEO</span><h2>Crear producto fabricable.</h2><p>La primera versión admite un componente principal; añade cada empaque o conjunto como materia prima para mantener el costo completo.</p></div></div>
        <form className={`${styles.card} ${styles.form}`} onSubmit={(event) => addRecipe(event, submit)}>
          ...
          <label>Clasificación<select className={styles.input} name="risk" defaultValue="LOW"><option value="LOW">Bajo riesgo · autorizado</option><option value="REVIEW_REQUIRED">Revisión regulatoria</option><option value="BLOCKED_AT_HOME">Bloqueado en residencia</option></select></label><label>Precio retail *<input className={styles.input} name="retailPrice" type="number" min="0.01" step="0.01" required/></label><label>Componente principal *<select className={styles.input} name="materialId" required defaultValue=""><option value="" disabled>Seleccionar material</option>{data.materials.map((item) => <option key={item.id} value={item.id}>{item.sku} · {item.name}</option>)}</select></label>
          <label>Cantidad por unidad *<input className={styles.input} name="quantity" type="number" min="0.0001" step="0.0001" required/></label><label>Desperdicio %<input className={styles.input} name="wastePercent" type="number" min="0" step="0.1" defaultValue="3"/></label><label>Trabajo (minutos)<input className={styles.input} name="laborMinutes" type="number" min="0" step="1" defaultValue="10"/></label>
          ...
          <label>Overhead/unidad<input className={styles.input} name="overheadCost" type="number" min="0" step="0.01" defaultValue="0"/></label><label className={styles.full}>Declaración de etiqueta<input className={styles.input} name="labelStatement" defaultValue="No consagrado / no preparado ceremonialmente."/></label><label className={styles.full}>Control de calidad, una verificación por línea<textarea className={styles.textarea} name="qcChecklist" defaultValue={"Componentes y colores correctos\nEnsamblaje resistente\nCantidad y etiqueta verificadas\nEmpaque limpio y sin daños"}/></label>
      ...
      <section className={styles.section}><div className={styles.sectionHead}><div><span>COSTOS Y PRODUCTOS</span><h2>Economía por unidad.</h2></div></div><div className={styles.productList}>{data.recipes.map((recipe) => { const economics = recipeEconomics(recipe, data.materials); return <article className={`${styles.card} ${styles.productRow}`} key={recipe.id}><div><div className={styles.kicker}>{recipe.sku} · V{recipe.version} · {recipe.risk}</div><h2>{recipe.name}</h2><div className={styles.productMeta}>Materiales ${economics.materialCost.toFixed(2)} · trabajo ${economics.laborCost.toFixed(2)} · costo total <strong>${economics.unitCost.toFixed(2)}</strong> · retail ${recipe.retailPrice.toFixed(2)} · margen <strong>{economics.grossMargin.toFixed(2)}%</strong></div><p>{recipe.labelStatement}</p></div><span className={recipe.risk === "LOW" ? styles.ok : styles.warning}>{recipe.risk === "LOW" ? "FABRICABLE" : "HOLD"}</span></article>; })}{!data.recipes.length && <div className={styles.empty}>No hay productos propios configurados.</div>}</div></section>
...
function addRecipe(event: FormEvent<HTMLFormElement>, submit: (action: string, form: HTMLFormElement, payload: Record<string, unknown>) => Promise<void>) { event.preventDefault(); const form = event.currentTarget; const row = values(form); void submit("add_recipe", form, { ...row, lines: [{ materialId: row.materialId, quantity: row.quantity, wastePercent: row.wastePercent }], qcChecklist: String(row.qcChecklist ?? "").split("\n") }); }
